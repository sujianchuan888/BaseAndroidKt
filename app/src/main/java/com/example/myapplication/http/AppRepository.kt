...
import com.example.myapplication.info.LoginInfo
...
class AppRepository private constructor(private val mHttpDataSource: HttpDataSource) : BaseModel(),
    HttpDataSource {
    override fun sendCode(json: RequestBody?): Observable<BaseBean<LoginInfo?>?>? {
        return mHttpDataSource.sendCode(json)
    }

    override fun login(json: RequestBody?): Observable<BaseBean<LoginInfo?>?>? {
